...
def book_flight(user_id: int, name: str, flight_id: int, seat_class: str = 'economy') -> BookingOut:
    ...
    Requires user_id, name, flight_id, and optionally seat_class (economy, business, or galaxium).
...
def book_flight_endpoint(request: BookingRequest, db: Session = Depends(get_db)):
    ...
    Requires user_id, name, flight_id, and seat_class. Decrements available seats for the selected class if successful.
    """
